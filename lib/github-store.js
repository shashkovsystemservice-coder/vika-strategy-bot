...
const DEFAULT_ISSUE = 2;
const MARKER_START = '<!-- VIKA_STRATEGY_ANSWER';
const RECOVERY_MARKER_START = '<!-- VIKA_STRATEGY_RECOVERY';
const MARKER_END = '-->';
...
async function appendRawComment(body) {
...
async function appendRecord(record) {
  await appendRawComment(buildCommentBody(record));
}
...
export async function appendRecoveryMessage({ user, answerType, transcript, telegramMessageId, forwardedAt = null, forwardOriginType = null }) {
  const record = {
    schema: '1.1.0',
    event: 'recovery_unassigned',
    telegram_user_id: user.id,
    username: user.username || null,
    display_name: [user.first_name, user.last_name].filter(Boolean).join(' ') || null,
    created_at: new Date().toISOString(),
    ...
    forward_origin_type: forwardOriginType,
    answer_type: answerType,
    transcript,
  };
  const body = `${RECOVERY_MARKER_START}\n${JSON.stringify(record)}\n${MARKER_END}\n\n### Архивный ответ — без привязки к вопросу\n\n**От:** ${record.display_name || record.username || record.telegram_user_id}\n\n**Исходная дата:** ${forwardedAt || 'не передана Telegram'}\n\n**Тип:** ${answerType}\n\n${transcript}`;
  await appendRawComment(body);
  return record;
}
